...
Once that bit is generated, it will be compressed to reset every context for the following prediction. Here, it is essential to update models in a way that if the same context is obteined in two different samples, probabilities must be the same, otherwise it could compute and propagate errors. Seeing that, it is necessary to turn off the training process and the weight adjustment of each model at generation time. This is possible because the source code of PAQ is available.


It is noted that granting too much freedom to our compressor might result in a large accumulation of bad predictions, leading to poor text generation. Therefore, it is proposed to make the text generation more conservative adding a parameter called ``temperature'' that reduces the possible range of the random number.

\begin{figure}[htb]
	\begin{center}
		\includegraphics[angle=0,width=0.5\textwidth]{temp2.png}
		\caption{Example of a temperature with value 0.8.
			The range of possible values of temperature is $[0.5; 1]$.}
		\label{fig1}
	\end{center}
\end{figure}

On maximum temperature, the random number will be generated in the interval [0,1) (i.e. it is the same as not using temperature parameter), giving the compressor maximum degree of freedom to make mistakes, whereas, when this parameter is 0.5, which is the minimum value, the random number will always be 0.5, removing the degree of freedom to commit errors (in this scenario, the highest probability symbol will be generated). 

When temperature is around 0.5 the results are very legible even if they are not as similar to the original text (according to the proposed metrics. This will be explained in further sections). It can be seen at the following randomly generated Harry Potter's snippet:
\newline
\begin{displayquote}
...
A simple transformation is applied to each text in order to compute metrics. It consists in counting the number of occurrences of each n-gram in the input (i.e. every time a n-gram ?WXYZ? is detected, it increases its number of occurrences). 
...
This metric is computed using 4-grams.
...
This metric is computed using 10-grams.
...
In this research, similarity between the input text (for example, The Complete Works of William Shakespeare) and an automatic generated text sample was measured using a local context and a global one. 
...
\begin{figure}[hb]
	\begin{center}
		...
		\caption{The effect of the chosen seed in the Chi Squared metric. In Orange the metric variation by temperature using a random seed. In Blue the same metric with a chosen seed.}
		\label{fig3}
	\end{center}
\end{figure}


In some cases the compressor generated text that was surprisingly well written. This is an example of random text generated by PAQ8L after compressing "Harry Potter"
\newline
\newline
...
\section{Effect of temperature parameter}

The values of each metric varies in function of the temperature parameter. As previously mentioned, a value of 0.5 means that a compressor always samples the symbol (0 or 1) with highest probability. However, it does not mean that the results obtained when computing the metrics are better. In~figure \ref{fig2} there is evidence that best values for Jaccard similarity for a given text are not obtained when temperature is 0.5. 
