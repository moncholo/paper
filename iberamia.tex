...
Recurrent neural networks have been used for automatic text generation \cite{Karpathy}. In this tasks, RNNs are trained with texts (or books) in order to sample new characters according to what it learned from those texts. Training a RNN with Shakespare's works can produce the following example:

\begin{displayquote}[width=0.5]
PANDARUS: \\
...
This module's goal is to automatically generate text with a PAQ series compressor and compare results against results generated  it with a Recurrent neural network's results \cite{Karpathy}, using specifics metrics and scenarios.
