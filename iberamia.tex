...
For this research, a  Long Short-Term Memory (LSTM) network has been used. A LSTM is a novel recurrent network architecture in conjunction with an appropiate gradient-based learning algorithm \cite{Hochreiter}. 

In order to sample new text, each character of the training text is represented as a vector using 1-of-k encoding (i.e. all zero except for a single one at the index of the character in the vocabulary), and this vector is fed into the RNN. Once the network is trained, a character is fed into it and a you will get a distribution over what characters are likely to come next. Once the distribution is given, a character is sampled from that distribution and then it is fed right back in to get the next letter \cite{Karpathy}.
As mentioned in Figure \ref{fig2}, a temperature parameter is also used when sampling text with the RNN used in this research where lower values will give more conservative results whereas using higher values will give more diversity but at cost of more mistakes.

Not only the temperature but also the number of layers and the size of the network are important parameters and they can vary. In fact, varying them is a common practice to find good models. In this research, the network has been trained several times varying such parameters. After each training process, many texts where sampled.  
...
Tables \ref{tab:localchi}, \ref{tab:localtotal} and \ref{tab:localjaccard} show local similarity results. It can be noticed that the results obtained using a PAQ compression algorithm are better than the ones obtained by the RNN execpting Poe, Shakespare and Game of Thrones because of a subtle reason that will be explained in the conclusions.
