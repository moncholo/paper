...
For many authors there is a very strong relationship between Data Compression and Artificial Intelligence\cite{Franz}\cite{Ofir}. Data Compression is about making good predictions \cite{Vitany2} which is also the goal of Machine Learning, a field of Artificial Intelligence.
...
\begin{displayquote}
	...
	\label{exampleee1}\textit{``The piano part was so simple it could have been picked out with one hand while the player whacked away at the gong with the other. This is one of the most bewilderedly tranceï¿½state inducing bad movies of the year so far for me.''}
	\newline
\end{displayquote}
...
Recurrent Neural Networks have been used for Automatic Text Generation \cite{Karpathy} \cite{Graves}. On this task, a RNN is trained with texts (or books) in order to sample new characters according to those texts' patterns. Readers may think that the following example was written by Shakespeare but it was not, a RNN was trained with Shakespeare's works and produced it \footnote{The source code for this network is available in \cite{Karpathy}} :
...
In this module, Karpathy's char-RNN \cite{Karpathy} and PAQ8L compressor will be used to generate automatic text. The results will be compared to determine which of them performs better. In the following sections, different scenarios and metrics will be used to make such comparison.
...
\begin{displayquote}
Where $P(1|c)$\footnote{The probability $P(0|c)$ can be interpreted as $1-P(1|c)$ } is the probability of bit 1 with context "c",
\newline
...
\newline
$S_i$\footnote{The signal $S_i$ is often computed as $S_i = stretch(P_i(1|c))$} is a signal that derives from $P_i(1|c)$
\end{displayquote}
...
\subsubsection{Generalized Jaccard Similarity}\label{subjaccard}
...
As suggested in "Generating sequences with recurrent neural networks"\cite{Graves}, Figure \ref{figrnn}  illustrates a basic recurrent neural network prediction architecture. An input vector sequence $x = (x_1, . . . , x_T)$ is passed through weighted connections to a stack of $N$ recurrently connected hidden layers to
...
Instead of using Stanford's Neural Network for Sentiment Analysis used in section \ref{sentimentRNN}, a  Long Short-Term Memory (LSTM) network called char-RNN has been used for sampling text\cite{Karpathy}. A LSTM is a novel recurrent network architecture in conjunction with an appropiate gradient-based learning algorithm \cite{Hochreiter}.
...
In this section, the results of both algorithms are shown with the purpose of doing an evaluation of how much similar the results are to the original inputs.  According to section \ref{aboutSamples}, similarity between texts can be computed using a local stage or a global one. The following subsections show the results for both stages.
...
\begin{table}[!hbt]
	\begin{center}
		...
		\begin{tabular}{|l|l|l|}\hline
			...
			Math Collection & \textbf{0.2100} & 0.1626 \\
			War and Peace & \textbf{0.0753} & 0.0689 \\
			Linux Kernel & \textbf{0.0738} & 0.0713 \\ \hline
		\end{tabular}
	\end{center}
\end{table}
In order to understand how tables must be read, consider as an example the first row of Table \ref{tab:localjaccard}. In such table, the Jaccard Similarity has been used within a local stage. As you could see in section \ref{subjaccard}, higher values mean that there is more similarity between two texts. The value 0.0638 (RNN) is higher than 0.06118 (PAQ) and it means that given local stage and Jaccard Similarity, RNN samples are more similar to the input text than the PAQ's ones.

It can be noticed that the results obtained using PAQ compression algorithm are better than the ones obtained by the RNN execepting Poe and Game of Thrones.
