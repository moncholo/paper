...
Recurrent neural networks have been used for automatic text generation \cite{Karpathy}. In this tasks, RNNs are trained with texts (or books) in order to sample new characters according to what it learned from those texts. Training a RNN with Shakespeare's works can produce the following example:
...
Readers may think that the previous example was written by Shakespeare but it was not. 
...
In this module, a recurrent neural network and a PAQ compressor will be used to generate text and the results will be compared so that to determine which of them performs better. In the following sections, different scenarios and metrics will be used to make such comparison.
...
When temperature is around 0.5 the results can be very legible even if they are not as similar to the original text (according to the proposed metrics). It can be seen at the following randomly generated Harry Potter's snippet:
\newline
\begin{displayquote}
...
\subsection{Results}
Turning off the training process and the weights adjustment of each model freezes the compressor's global context on the last part of the training set. As a consequence of this event, the last piece of the entry text will be considered as a "big seed" for the compression algorithm.
