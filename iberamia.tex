...
\begin{table}[!hbt]
	...
	\end{center}
\end{table}
In order to understand how tables must be read, consider as an example the first row of Table \ref{tab:localjaccard}. In such table, the Jaccard Similarity has been used within a local stage. As you could see in section \ref{subjaccard}, higher values mean that there is more similarity between two texts. The value 0.0692 (PAQ) is higher than 0.0608 (RNN) and it means that given local stage and Jaccard Similarity, PAQ samples are more similar to the input text than the RNN's ones.

It can be noticed that the results obtained using a PAQ compression algorithm are better than the ones obtained by the RNN execepting Poe and Game of Thrones.
